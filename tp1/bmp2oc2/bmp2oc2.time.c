...
//	int size = 19;
	...
	tablaApariciones = calcularApariciones(buffer, size);
		...
		double milli_pre = tv.tv_sec * 1000 + tv.tv_usec / 1000.0;

		int xkcd;
		for(xkcd=0;xkcd<1000;xkcd++){
			//Encontramos j
			tablaApariciones = calcularApariciones(buffer, size);
		}

		gettimeofday (&tv, NULL);
		double milli_post = tv.tv_sec * 1000 + tv.tv_usec / 1000.0;

		double time_past = (milli_post - milli_pre)/1000;
		printf("Tiempo: %f\n", time_past);
